...
// Add this to the bottom of your createBinaryTree method in nodes.js
this.createBinaryTree = function(arr) {
    treeContainer = createContainer("binary-tree", arr);
    start = treeContainer.attr("width") / 2;
    ...
    while (i < arr.length) {
      ...
      this.addNode(node);
      smoothAddNode(node); // Smooth animation for node
      smoothAddText(node); // Smooth animation for text
      ++i;
    }
